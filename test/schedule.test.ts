...
describe("SubagentScheduler — fire path", () => {
  ...
  it("fire passes background runtime metadata to manager.spawn", () => {
    scheduler.addJob({
      name: "every-1s", description: "x", schedule: "1s",
      subagent_type: "general-purpose", prompt: "x", thinking: "high",
    });
    ...
    expect(optsArg.invocation).toEqual(expect.objectContaining({
      thinking: "high",
