...
### Added
- **Ordered per-agent and default model fallback chains.** Custom agents can declare `fallback_models`, including `false` to disable inherited defaults, while global/project `subagents.json` can provide `defaultFallbackModels`. Pi's same-model retries run first; exhausted provider/model assistant errors then retry the same invocation in the same child session on the next explicitly configured model, without changing Pi's persisted default model. Spawn and resume share the behavior, unavailable or ambiguous candidates are skipped with bounded attempt diagnostics, all-candidate failure is fail-closed, provider-qualified names never jump providers implicitly, context overflow remains owned by Pi compaction, and tool/task/abort/max-turn outcomes do not switch models. Requires Pi 0.80.5 or later.
...
### Fixed
- **Agent model overrides and tool/RPC contracts now follow Pi's real execution semantics.** A trimmed nonblank function-call `model` overrides agent frontmatter, blank values are treated as omitted, resume bypasses spawn-only model validation, and equal-best fuzzy matches are rejected as ambiguous instead of depending on registry order. Scheduled explicit/frontmatter models are persisted and re-resolved fail-closed, so unavailable models no longer silently inherit another model. All six public top-level/nested schemas are strict and use Google-compatible string enums, nested and top-level invocation rejections throw through Pi's model-visible `isError` channel, and cross-extension RPC protocol v3 validates malformed requests and exposes only a serializable public spawn-option allowlist. Full, compact, and custom tool descriptions now agree on precedence, resume exclusivity, and automatic background completion.
