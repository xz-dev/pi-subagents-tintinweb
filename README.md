...
Frontmatter is authoritative for `thinking`, `max_turns`, `inherit_context`, `run_in_background`, and `isolated`; `Agent` parameters fill only fields the config leaves unspecified. `model` is a deliberate exception: a nonblank function-call value overrides frontmatter, while blank or whitespace means omitted. `isolation` is also an explicit-state exception: `isolation: "off"` disables an agent's `worktree` default. A resume uses only `resume` and `prompt`; spawn-only fields are ignored.

**Forgiving `model:` resolution.** A model selector is matched against pi's model registry tolerantly: `.` and `-` are treated as equivalent in version numbers (`claude-haiku-4.5` ≡ `claude-haiku-4-5`), and a trailing `-YYYYMMDD` date stamp is optional (`anthropic/claude-haiku-4-5-20251001` matches an undated registry id and vice-versa). A provider-qualified selector stays within that provider; cross-provider choices must be listed explicitly in the fallback chain. Equal-best fuzzy matches are rejected as ambiguous instead of selecting registry order. For immediate spawns, an unavailable frontmatter pin retains the existing parent-model fallback; explicit call overrides and scheduled model selectors fail closed. `/agents → Agent types` flags an unavailable frontmatter pin and shows the resolved target `(→ provider/id)` when resolution lands on a different provider or version than configured. (This is distinct from [Model Scope](#model-scope) enforcement, which matches the `enabledModels` allowlist by *exact* entry.)
...
**Design:** `scopeModels` is a guardrail against the orchestrator picking unexpected models at runtime, not a hard policy against user-level config. A nonblank caller model is therefore checked as the explicit effective override; when no override is supplied, frontmatter pins and configured fallback candidates run even when out of scope (with a visible warning).
